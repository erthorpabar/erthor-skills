...
def generate_image(prompt, width, height,
    ...
    # url ="http://192.168.31.34:6002"  # 测试打开
    ...
    def on_open(ws): # 在打开的时候提交生成任务
        ...
        try:
            # 打开工作流文件
            with open('services/comfyui_api/two_character_02_api.json', 'r',encoding='utf-8') as f:
            ...
            work_flow['26']['inputs']['noise_seed'] = random.randint(0, 4294967295) # 设置随机种子
            work_flow['68']['inputs']['input_str'] = prompt # 提示词
            work_flow['100']['inputs']['input_int'] = width # 宽度
            work_flow['101']['inputs']['input_int'] = height # 高度

            work_flow['11']['inputs']['unet_name'] = checkpoint_name # 模型名称
            work_flow['106']['inputs']['lora_name'] = lora_name # 模型精度

            work_flow['51']['inputs']['image'] = left_image # 左边的人物图片地址
            work_flow['103']['inputs']['image'] = right_image # 右边的人物图片地址
...
# prompt = '''The woman was on the left side of the picture,
# black beret,
# Long golden hair,
# Black turtleneck sweater,
# white pants,

# and the woman was on the right side of the picture,
# Light Yellow drill_hair,
# Amber hoodie,
# Yellow Green shorts,
# in landscape.

# The left woman heard the shouting, ran to the window, looked down, and saw the right woman downstairs.'''
# a = generate_image(prompt=prompt, width=1536, height=1024,
#                    checkpoint_name="flux-dev-fp8.safetensors",
#                    lora_name='flux_lora/推文韩漫都市言情_V1.0.safetensors',
#                    left_image="ComfyUI_temp_baulh_00005_.png",
#                    right_image="ComfyUI_temp_baulh_00004_.png")
# print(a)