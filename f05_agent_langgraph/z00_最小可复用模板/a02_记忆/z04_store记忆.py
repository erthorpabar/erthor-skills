...
# 用于存放用户信息或偏好
# store = InMemoryStore()
store = SqliteStore.from_conn_string("store.db")

# ===========================================
print("👋 欢迎使用AI对话助手 输入 'quit' 退出对话。\n")
# 加载历史记录
with memory as memory, store as store:
    ...
    print(profile_info)
    system_prompt = f"你是一个助手。用户信息: {profile_info}"
    ...
    while True:
        ...
        result = app.invoke(
            {"messages": [
                SystemMessage(content=system_prompt), # 自动加在最前边 会永久合并到message中
