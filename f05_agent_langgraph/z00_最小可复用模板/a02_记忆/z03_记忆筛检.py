...
# 1 固定长度记忆窗口
def memory_window(state: MessagesState):
    messages = state["messages"]
    if len(messages) > 10:
        recent_msgs = messages[-10:]
        # 提取id
        keep_ids = {m.id for m in recent_msgs} 
        # 删除其他消息
        memory_window = [RemoveMessage(id=m.id) for m in messages if m.id not in keep_ids]
        return {"messages": memory_window}
    return {}

# 2 记忆总结
def memory_summary(state: MessagesState):
    messages = state["messages"]
    if len(messages) > 10:
        recent_msgs = messages[-10:]
...
# 4 定期清理旧对话thread_id
def clean_thread_id(state: MessagesState):
    pass
...
with memory as memory:
    # 编译图
    app = graph.compile(checkpointer=memory) # 在节点内部使用
